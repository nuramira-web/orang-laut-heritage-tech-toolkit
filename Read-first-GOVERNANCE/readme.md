...
What is here
FilePurposeLOCAL_CONTEXTS_LABELS_REFERENCE.md Full reference for the four Local Contexts Notices and twenty TK Labels, with guidance on when each applies.CONSENT_AGREEMENT_TEMPLATE.mdPlain-language consent form for contributors and tradition-bearers. Bilingual (English / Bahasa Melayu) skeleton.METADATA_SCHEMA.mdThe cataloging schema used across all toolkit modules, including Local Contexts fields.catalog_entry_template.mdA blank, fillable template for a single archive item.catalog_entry_example.mdA worked example showing the template populated.
